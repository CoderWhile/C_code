...
int Seat_Perst_InsertBatch(seat_list_t list) {
	...
	//批量分配主键
	long key = EntKey_Perst_GetNewKeys(SEAT_KEY_NAME, total_count); //为座位分配获取
	if (key <= 0)			//主键分配失败，直接返回
		return 0;
	...
	long current_key = key;  // 当前要分配的主键
	List_ForEach(list, p) {
		p->data.id = current_key++;
		int write_result = fwrite(&p->data, sizeof(seat_t), 1, fp);
		if (write_result == 1) {
			rtn++;
		}
	}
